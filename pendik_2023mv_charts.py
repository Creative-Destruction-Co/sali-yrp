"""Pendik 2023 MV — 4 panel yan yana yatay bar (Pendik + 3 gelir grubu).
Hesap: kayitli secmen sayisi uzerinden. Diger + Gecersiz + Sandiga Gitmeyen dahil."""
import pandas as pd
...
# Sandık-bazli detayli xlsx (kayitli secmen + gecerli + gecersiz + parti oylari)
xls = pd.read_excel(ROOT/'PENDİK/Secim/2023_MİLLETVEKİLİ_GENEL.xlsx')

PALETTE = {
    'AK PARTİ':'#FFC436','CHP':'#FE0000','MHP':'#11009E','İYİ PARTİ':'#52D3D8',
    'YEŞİL SOL PARTİ':'#8B1874','YENİDEN REFAH':'#65B741','ZAFER PARTİSİ':'#765827',
    'TİP':'#C70039','Diğer':'#9a9a9a','Geçersiz':'#5d5d5d','Sandığa Gitmeyen':'#bdb1a0',
}
PARTIES = ['AK PARTİ','CHP','MHP','İYİ PARTİ','YEŞİL SOL PARTİ','YENİDEN REFAH','ZAFER PARTİSİ','TİP']
...
# Mahalle ismini normalize et (tip.csv ile eslesme icin)
import sys
...
for label, sub, n in panels:
    ...
    parti_top = sum(sub[p].sum() for p in PARTIES)
    diger = gecerli - parti_top
...
svg = (f'<?xml version="1.0" encoding="UTF-8"?>\n'
       ...
       + '\n'.join(elems) + '\n</svg>\n')
out = OUT/'pendik_2023mv_4panel.svg'
