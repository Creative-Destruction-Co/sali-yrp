"""YRP 2023 MV mahalle haritası — asimetrik diverging (lejant gerçek puanlar)."""
...
df = pd.read_csv(ROOT/'PENDİK/oranlar/oran_2023_MİLLETVEKİLİ_GENEL.csv')
df['_avg'] = df['YENİDEN REFAH'] / df['n_sandik']
ilce_real = float((df['_avg'] * df['gecerli_oy_toplam']).sum() / df['gecerli_oy_toplam'].sum() * 100)
# Ortak skala (15 ve 16 yansilari icin)
ilce = 4.22  # (3.93 + 4.52)/2
print(f'Pendik 2023 MV YRP gerçek ortalaması: %{ilce_real:.2f}, ortak midpoint: %{ilce:.2f}')
...
vmin = 1.44  # ortak skala
vmax = 6.73  # ortak skala
print(f'vmin={vmin:.2f}, ilce={ilce:.2f}, vmax={vmax:.2f}')
...
svg = ('<?xml version="1.0" encoding="UTF-8"?>\n'
       ...
       '  <g class="polygons">\n' + '\n'.join(paths) + '\n  </g>\n</svg>\n')
out = OUT/'pendik_yrp_2023mv.svg'
out.write_text(svg, encoding='utf-8')
print(f'Wrote {out.name}: {len(svg)//1024}KB')
print(f'Lejant: vmin={vmin:.2f}, mid_neg={(vmin+ilce)/2:.2f}, ilce={ilce:.2f}, mid_pos={(ilce+vmax)/2:.2f}, vmax={vmax:.2f}')
